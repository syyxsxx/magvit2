...
def train_g_d(
  ...
  def loss_fn(params_d, params_g):
    ...
    print("*****realvideo*********")
    print(real_video.shape)
    print(config.vqgan.gradient_penalty)
...
def train(*, rng: jnp.ndarray, config: ml_collections.ConfigDict,
  ...
  if (start_step == 0  # Which means "no" checkpoint is restored!
    ...
    init_checkpoint_path = '/workspace/dir10/checkpoint_948960'
  ...
  with metric_writers.ensure_flushes(writer):
    for step in range(start_step + 1, total_steps + 1):
      with jax.profiler.StepTraceAnnotation('train', step_num=step):
        ...
        print('11111'*10)
        print('train_batch.shape', train_batch['inputs'].shape)

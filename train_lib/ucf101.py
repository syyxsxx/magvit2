"""Data generators for the ucf101 dataset."""
...
@scenic_datasets.add_dataset('ucf101')
def get_dataset(*,
                batch_size,
                eval_batch_size,
                num_shards,
  ...
  num_frames = dataset_configs.get('num_frames', 17)
  # need to fps / 8
  ...
  num_train_examples = scenic_dataset_utils.get_num_examples(
      'ucf101', 'train')
  num_eval_examples = scenic_dataset_utils.get_num_examples('ucf101',
                                                     'test') * num_eval_clips
  meta_data = {
      'num_classes': None,

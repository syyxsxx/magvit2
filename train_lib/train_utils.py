...
from videogvt.train_lib import ucf101
from videogvt.train_lib import custom_dataset

_CUSTOM_DATASET_TABLE = {'ucf101' : ucf101,
...
def get_dataset(
  ...
  if dataset_type == 'video':
    ...
    if dataset.train_iter:
      replace(dataset, train_iter=map(_inputs_add_t_dim, dataset.train_iter))
    if dataset.valid_iter:
      replace(dataset, valid_iter=map(_inputs_add_t_dim, dataset.valid_iter))
    if dataset.test_iter:
      replace(dataset, test_iter=map(_inputs_add_t_dim, dataset.test_iter))
    return dataset
  else:

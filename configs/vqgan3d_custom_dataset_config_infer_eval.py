...
from videogvt.configs import vqgan2d_custom_dataset_config_infer_eval

VARIANT = 'VQGAN/3D'


def get_config(config_str='H-eval'):
    """Returns the base experiment configuration."""
    version, *options = config_str.split('-')

    config = vqgan2d_custom_dataset_config_infer_eval.get_config(config_str)
    config.experiment_name = f'UCF101_{VARIANT}'
    model_class, model_type = VARIANT.split('/')

    # Overall
    config.batch_size = {'B': 2, 'L': 4, 'H': 4}[version]
    config.eval_batch_size = config.get_ref('batch_size') // 4
    config.num_training_epochs = {'B': 500, 'L': 2000, 'H': 500}[version]
    ...
    config.vqvae.codebook_size = {'B': 1024, 'L': 1024, 'H': 262144}[version]
    ...
    config.vqvae.upsample = 'depth_to_space'

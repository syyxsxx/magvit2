...
def get_config(config_str='B'):
  ...
  config.dataset_configs.data_augmentations = ['default']
  ...
  common_pps = (
      'value_range(0, 1)|copy("label", "labels")|keep("image", "labels")'
  )
  config.dataset_configs.pp_train = (
      f'decode_jpeg_and_inception_crop({config.image_size})|flip_lr'
      f'|{common_pps}'
  )
  config.dataset_configs.pp_eval = (
      f'decode|resize_small({config.image_size})'
      f'|central_crop({config.image_size})|{common_pps}'
  )
  ...
  config.vqvae.codebook_size = 262144
  ...
  config.vqvae.vector_quantizer_class = 'LFQuantizer'
  ...
  config.logging.checkpoint_kept = 200

...
class Conv(nn.Conv):
  ...
  def __call__(self, x):
    ...
    if self.custom_padding is not None:
      ...
      #print(pads )
      #print("##########kernel len#######")
      #print(len(self.kernel_size))
      
      if len(self.kernel_size) == 3:
        h_pad = [((self.kernel_size[1] - 1) // 2, self.kernel_size[1] // 2)]
        w_pad = [((self.kernel_size[2] - 1) // 2, self.kernel_size[2] // 2)]
        #pads = [(0, 0) + [0, 0] + h_pad + w_pad + (0, 0)]
        pads =  [(0, 0)] + [(0, 0)] + h_pad + w_pad + [(0, 0)]
      else:
        pads = [((k - 1) // 2, k // 2) for k in self.kernel_size]
        pads = [(0, 0)] + pads + [(0, 0)]
      ...
      data_shape[1] = data_shape[1] + (self.kernel_size[0] - 1)
      new_x = jnp.zeros(tuple(data_shape))
      #print(data_shape)
      #print(x.shape)
      index = self.kernel_size[0] - 1
      for i in range(0, index):
          new_x = new_x.at[:,i:i+1,:,:,:].set(x[:,0:1,:,:,:])
      new_x = new_x.at[:,index:,:,:,:].set(x)
      x = new_x
      #new_x[:,0:1,:,:,:] = x[:,0:1,:,:,:]
      #new_x[:,1:2,:,:,:] = x[:,0:1,:,:,:]
      #new_x[:,2:,:,:,:] = x
      #print("###############casual3d padding out ##########")
      #print(x.shape)

...
if __name__ == '__main__':
    ...
    input_spec = [(
        (-1, 17, 128, 128, 3),  # bs, t, h, w, 3
        jax.numpy.float32)]

    eval_batch={}
    image_path = './224_128/'
    images = []
    for image_name in sorted(os.listdir(image_path), key=lambda x: float(x.split('f')[1].split('_')[0])):
